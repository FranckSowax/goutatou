  ...
  return (
    <section className="rounded-lg bg-white p-4 shadow-xs">
        ...
        {prizes.map((prize) => (
          <li key={prize.id} className="flex items-center justify-between gap-3 border-b pb-2">
            ...
            <form action={updatePrize.bind(null, prize.id)} className="flex items-center gap-2 text-xs">
              ...
              />
              <button className="rounded-sm border px-2 py-1">Enregistrer</button>
            </form>
      ...
      <form action={createPrize} className="mt-4 grid grid-cols-3 gap-2 text-sm">
        <input name="label" required placeholder="Nom du lot" className="col-span-3 rounded-sm border p-2" />
        <input name="weight" required type="number" min="1" defaultValue={1} placeholder="Poids" className="rounded-sm border p-2" />
        <input name="stock" required type="number" defaultValue={-1} placeholder="Stock (-1 = illimité)" className="col-span-2 rounded-sm border p-2" />
        <button className="col-span-3 rounded-sm bg-neutral-900 p-2 text-white">Ajouter le lot</button>
      </form>
    </section>
  )
}

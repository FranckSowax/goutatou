...
export function AppShell({ items, title, footer, userEmail, children }: {
  ...
  return (
    <div className="min-h-screen bg-background md:h-screen md:p-3 lg:p-4 print:h-auto print:min-h-0 print:p-0">
      <div className="flex min-h-screen flex-col md:h-full md:min-h-0 md:flex-row md:overflow-hidden md:rounded-3xl md:border md:border-border md:bg-card md:shadow-sm print:block print:h-auto print:min-h-0 print:overflow-visible print:rounded-none print:border-none print:shadow-none">
        {/* Sidebar desktop — masquée à l'impression (POS3, ticket de commande) : seul le contenu
            de `main` (le reçu) doit sortir sur le papier. */}
        ...
        {/* Topbar mobile — masquée à l'impression (idem sidebar desktop). */}
        <div className="sticky top-0 z-20 border-b border-border bg-card md:hidden print:hidden">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-16 shrink-0 flex-nowrap items-center justify-between gap-3 border-b border-border bg-card px-4 md:px-6 print:hidden">
          ...
          <main className="flex-1 overflow-y-auto bg-background/60 p-4 md:p-6 print:overflow-visible print:bg-transparent print:p-0">{children}</main>
        </div>
      </div>
    </div>
  )
